...
def dev(model, dev_loader, idx2slot, idx2intent, results_writer):
    ...
    total_wrong = 0 
    error_logs = [] # Buffer for error messages
    ...
    for i, batch in enumerate(tqdm(dev_loader, desc="Evaluating")):
        ...
        for idx_in_batch in range(len(pred_slot)):
            ...
            if intent_is_wrong or slots_are_wrong:
                ...
                error_string = (
                    f"Example {total_wrong}\n"
                    f"Input IDs: {input_ids_list}\n"
    ...
    # MODIFIED: Return total_wrong and error_logs (wrong_pred_list)
    return intent_acc, slot_f1, sent_acc, ave_loss_intent, ave_loss_slot, total_wrong, error_logs
...
def run_train(train_data_file, dev_data_file):
    ...
    for epoch in trange(config.epoch, desc="Epoch"):
        ...
        # We only call dev to get metrics for logging
        
        # Call dev, but ignore the error logs (last 2 returned values)
        (intent_acc, slot_f1, sent_acc, 
         ave_loss_intent, ave_loss_slot, 
         _, _) = dev(model, dev_loader, idx2slot, idx2intent)
    ...
    print("\nRunning final evaluation on dev set with best model...")
    # Load the best model saved during training
    model = torch.load(config.model_save_dir + config.model_path, map_location=device, weights_only=False)
    
    final_dev_results_path = os.path.join(config.model_save_dir, "final_dev_wrong_predictions.txt")
    
    # Call dev to get all metrics and logs
    (dev_intent_acc, dev_slot_f1, dev_sent_acc, 
     _, _,  # Ignore average losses for this report
     total_dev_wrong, dev_error_logs) = dev(model, dev_loader, idx2slot, idx2intent)
    
    print(f"Final Dev Metrics - Acc: {dev_intent_acc:.4f}, Slot F1: {dev_slot_f1:.4f}, Sent Acc: {dev_sent_acc:.4f}")

    # Write the final dev errors file in the requested format
    with open(final_dev_results_path, 'w', encoding='utf-8') as f_dev_wrong:
        f_dev_wrong.write("***** Wrong Predictions (Dev Set) *****\n")
        f_dev_wrong.write(f"Total wrong examples: {total_dev_wrong}\n")
        f_dev_wrong.write("========================================\n\n")
        f_dev_wrong.writelines(dev_error_logs)
...
def run_test(test_data_file):
    ...
    total_test_wrong = 0
    error_logs = [] # List to store error strings
    ...
    for i, batch in enumerate(tqdm(test_loader, desc="Evaluating Test Set")):
        inputs, char_lists, slot_labels, intent_labels, masks, = batch
        if use_cuda:
    ...
    with open(test_results_file_path, 'w', encoding='utf-8') as f_test_wrong:
        ...
        f_test_wrong.write("========================================\n\n")
        f_test_wrong.writelines(error_logs)

    # --- Calculate and Print Metrics (unchanged) ---
    # slot f1, p, r
    slot_metrics = SlotMetrics(true_slots, pred_slots)
